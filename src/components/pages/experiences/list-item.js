...
const styleSheet = createStyleSheet('ExperiencesListItem', theme => ({
    padding: {
        padding: 30
    },
    default: {
...
class ExperiencesListItem extends Component {
    render() {
        const { classes, experience, color } = this.props;
        const align = window.screen.width > 800 ? 'flex-start' : 'center';
        const url = `experiencias/${experience.id}`;
...
export default withStyles(styleSheet)(ExperiencesListItem);
